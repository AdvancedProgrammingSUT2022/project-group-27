...
public class CityPanel extends Menus {
    ...
    private void setBuildingsAndUnits(City city, VBox vBox) {
        for (BuildingsType building: city.getBuildings()) {
            Label label = new Label(building.name());
            Circle circle = new Circle();
            circle.setRadius(30);
            circle.setFill(building.getImage());
    ...
    private void changeConstruction(City city, VBox vBox) {
        ...
        comboBox.setOnAction((event) -> {
            ...
            Message message = controller.changeConstruction(city, selection);
            showAlert(message);
        });
    }
    ...
    private void buildBuilding(City city, VBox vBox) {
        ...
        comboBox.setOnAction((event) -> {
            ...
            Message message = controller.buildBuilding(city, selection[0]);
            showAlert(message);
        });
    }
    ...
    private void buildUnit(City city, VBox vBox) {
        ...
        comboBox.setOnAction((event) -> {
            String selection = comboBox.getSelectionModel().getSelectedItem();
            Message message = controller.buildUnit(city, selection);
            showAlert(message);
        });
    }
    ...
    private void showRemainTimes(City city, VBox vBox) {
        ...
        if (city.getConstruction() != null) vBox.getChildren().add(new Label(city.getConstruction() +
