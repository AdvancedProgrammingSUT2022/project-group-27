...
public class DiplomacyPanel extends Menus {
    ...
    public void declareWar(MouseEvent mouseEvent) {
        ...
        for (Player player: Player.getAllPlayers()) {
            if (player != Player.getPlayerByUser(UserController.getInstance().getUsername())) users.add(player.getUser());
        }
        ...
        submit.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                Player other = Player.getPlayerByUser(comboBox.getSelectionModel().getSelectedItem());
