...
public class TradePanel extends Application {
    ...
    private void setSendingObject() {
        ...
        for (StrategicResource strategicResource: Player.getPlayerByUser(UserController.getInstance().getUsername()).getAllStrategicResources()) {
            products.add(strategicResource.name());
        }

        for (LuxuryResource luxuryResource: Player.getPlayerByUser(UserController.getInstance().getUsername()).getAllLuxuryResources()) {
            products.add(luxuryResource.name());
        }
    ...
    public void submit(MouseEvent mouseEvent) {
        ...
        try {
            ...
            if (sendingObject.getSelectionModel().getSelectedItem().equals("gold") &&
                    Player.getPlayerByUser(UserController.getInstance().getUsername()).getGold() < sendNumber) throw new RuntimeException();
        ...
        new Trade(UserController.getInstance().getUsername(), comboBox.getSelectionModel().getSelectedItem(),
