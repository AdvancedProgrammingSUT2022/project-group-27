...
public class RiverForCreateMap extends Polygon {
    private Ground firstGround,secondGround;
    public RiverForCreateMap(Ground groundRectangle1,Ground groundRectangle2){
        ...
        if (River.doWeHaveGroundBetweenTheseTwoGrounds(firstGround,secondGround)) this.setFill(Color.AQUA);
        if (y1!=y2) {
        ...
        this.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                if (River.doWeHaveGroundBetweenTheseTwoGrounds(firstGround,secondGround)){
                    River.removeRiver(River.GetRiverByGrounds(firstGround,secondGround));
                    setFill(Color.WHITE);
                }
                else{
                    River river=new River(firstGround,secondGround);
                    setFill(Color.AQUA);
                }
            }
        });
